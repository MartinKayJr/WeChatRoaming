...
import cn.martinkay.wechatroaming.settings.dsl.item.*
import cn.martinkay.wechatroaming.utils.LayoutHelper.MATCH_PARENT
import cn.martinkay.wechatroaming.utils.SyncUtils
import cn.martinkay.wechatroaming.utils.SyncUtils.async
import cn.martinkay.wechatroaming.utils.SyncUtils.runOnUiThread
...
class SettingsMainFragment : BaseRootLayoutFragment() {
    ...
    override fun onAttach(context: Context) {
        ...
        val desc = cn.martinkay.wechatroaming.settings.dsl.FunctionEntryRouter.findDescriptionByLocation(location)
        ...
        title = mFragmentDescription.name ?: "BlackSpider"
    ...
    private fun convertEndNode(context: Context, endNode: IDslItemNode): DslTMsgListItemInflatable {
        if (endNode is UiItemAgentDescription) {
            ...
            return SimpleListItem(endNode.identifier, endNode.name ?: endNode.toString(), null).apply {
                onClickListener = {
                    val targetLocation = cn.martinkay.wechatroaming.settings.dsl.FunctionEntryRouter.resolveUiItemAnycastLocation(
                        arrayOf(
                            cn.martinkay.wechatroaming.settings.dsl.FunctionEntryRouter.Locations.ANY_CAST_PREFIX, endNode.identifier
                        )
